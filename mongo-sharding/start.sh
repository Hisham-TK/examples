...
export CURRENT_IP_ADDRESS=192.168.2.11
...
docker stop mongos -t 1 && docker rm mongos -f && docker volume rm mongos_mongos
...
echo After 10 seconds will config all replica sets && sleep 10
...
echo After 5 seconds will Add shards to Mongs && sleep 10
...
echo After 20 seconds will add data to shardingDb && sleep 10
mongo --port $MONGOS_CONTAINER_PORT <./hashedSharding.js
#  && echo Range-base collection data are inserted
mongo --port $MONGOS_CONTAINER_PORT <./rangeBaseSharding.js
#  && echo Hashed collection data are inserted
